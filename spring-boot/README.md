 ...
 * we can customize that inside a custom requestracing handler. 
 
 * for the Go counterpart library, we extract the tenant, requestID as Span tag and the operationID as the Span Operation name (assuming that we have good operationID) to minimize the integration effort (inside a a middleware or http client roundtripper) 

 * the Span decorators can extract items like X-REQUEST-ID from http request and translate that to span tags. 
 The current opensource library uses HTTP MEthod as Span operation name (e.g. like GET which does not give much contest of what the Span is actually doing)  For example, the HellowController method "hello"  gives more meaning than "GET" as span operation name, "hello" is not available to http servelet request.
We can use Span decorators to overwrite or some Servlet Filters to overwrite the default libray behaviour.
...
* Location + branch:
  https://github.com/etsangsplk/opentracing-java-examples/tree/lightstep`# Spring boot application and Opentracing Integration POC
## Small POC Spring boot wth opentracing application 
